...
water_use_melt <- data.frame(value    = c(all.sfds$avg.30day.use, all.sfds$avg.30day.use_optimal, all.sfds$avg.30day.use_optimal_MUC213, all.sfds$avg.30day.use_optimal_MUC426),
                             variable = rep(factor(c('Current use', 'Utility maximizing', 'Socially optimal, MUC = $2.13/tgal', 'Socially optimal, MUC = $4.26/tgal'),
                                                   levels = c('Current use', 'Utility maximizing', 'Socially optimal, MUC = $2.13/tgal', 'Socially optimal, MUC = $4.26/tgal')),
                                            each = nrow(all.sfds)))
...
ggplot(data = water_use_melt) + geom_boxplot(aes(x = variable, y = value/1000),
  ...
  annotate("text", x = 1:4,
           y     = c(      median(water_use_melt[water_use_melt$variable == 'Current use',                        1], na.rm = TRUE)/1000+0.5,
                           median(water_use_melt[water_use_melt$variable == 'Utility maximizing',                 1], na.rm = TRUE)/1000+0.5,
                           median(water_use_melt[water_use_melt$variable == 'Socially optimal, MUC = $2.13/tgal', 1], na.rm = TRUE)/1000+0.5,
                           median(water_use_melt[water_use_melt$variable == 'Socially optimal, MUC = $4.26/tgal', 1], na.rm = TRUE)/1000+0.5),
           
           label = c(format(round(median(water_use_melt[water_use_melt$variable == 'Current use',                        1], na.rm = TRUE)/1000, 2), nsmall = 2),
                     format(round(median(water_use_melt[water_use_melt$variable == 'Utility maximizing',                 1], na.rm = TRUE)/1000, 2), nsmall = 2),
                     format(round(median(water_use_melt[water_use_melt$variable == 'Socially optimal, MUC = $2.13/tgal', 1], na.rm = TRUE)/1000, 2), nsmall = 2),
                     format(round(median(water_use_melt[water_use_melt$variable == 'Socially optimal, MUC = $4.26/tgal', 1], na.rm = TRUE)/1000, 2), nsmall = 2)))

...
rm(optimal_consumption, Q_intercept_fcn)
...
# elasticity -0.02; MUC = 0, MUC = 1.84, and MUC = 4.26

elasticity_demand <- -0.020

all.sfds$DWL_optimal_elasticity020_MUC000 <- NA
for(i in 1:nrow(all.sfds)){all.sfds$DWL_optimal_elasticity020_MUC000[[i]] <- deadweight_loss(q_actual = all.sfds$avg.30day.use[[i]], q_optimal = all.sfds$avg.30day.use_optimal_elasticity020_MUC000[[i]],
                                                                                             A = all.sfds$A_elasticity020[[i]], no.sewer = all.sfds$no.sewer[[i]])}
all.sfds$DWL_optimal_elasticity020_MUC184 <- NA
for(i in 1:nrow(all.sfds)){all.sfds$DWL_optimal_elasticity020_MUC184[[i]] <- deadweight_loss(q_actual = all.sfds$avg.30day.use[[i]], q_optimal = all.sfds$avg.30day.use_optimal_elasticity020_MUC184[[i]],
                                                                                             A = all.sfds$A_elasticity020[[i]], no.sewer = all.sfds$no.sewer[[i]])}
all.sfds$DWL_optimal_elasticity020_MUC368 <- NA
for(i in 1:nrow(all.sfds)){all.sfds$DWL_optimal_elasticity020_MUC368[[i]] <- deadweight_loss(q_actual = all.sfds$avg.30day.use[[i]], q_optimal = all.sfds$avg.30day.use_optimal_elasticity020_MUC368[[i]],
                                                                                             A = all.sfds$A_elasticity020[[i]], no.sewer = all.sfds$no.sewer[[i]])}
...
all.sfds$DWL_optimal_elasticity060_MUC000 <- NA
for(i in 1:nrow(all.sfds)){all.sfds$DWL_optimal_elasticity060_MUC000[[i]] <- deadweight_loss(q_actual = all.sfds$avg.30day.use[[i]], q_optimal = all.sfds$avg.30day.use_optimal_elasticity060_MUC000[[i]],
                                                                                             A = all.sfds$A_elasticity060[[i]], no.sewer = all.sfds$no.sewer[[i]])}
all.sfds$DWL_optimal_elasticity060_MUC184 <- NA
for(i in 1:nrow(all.sfds)){all.sfds$DWL_optimal_elasticity060_MUC184[[i]] <- deadweight_loss(q_actual = all.sfds$avg.30day.use[[i]], q_optimal = all.sfds$avg.30day.use_optimal_elasticity060_MUC184[[i]],
                                                                                             A = all.sfds$A_elasticity060[[i]], no.sewer = all.sfds$no.sewer[[i]])}
all.sfds$DWL_optimal_elasticity060_MUC368 <- NA
for(i in 1:nrow(all.sfds)){all.sfds$DWL_optimal_elasticity060_MUC368[[i]] <- deadweight_loss(q_actual = all.sfds$avg.30day.use[[i]], q_optimal = all.sfds$avg.30day.use_optimal_elasticity060_MUC368[[i]],
                                                                                             A = all.sfds$A_elasticity060[[i]], no.sewer = all.sfds$no.sewer[[i]])}
...
all.sfds$DWL_optimal_elasticity326_MUC000 <- NA
for(i in 1:nrow(all.sfds)){all.sfds$DWL_optimal_elasticity326_MUC000[[i]] <- deadweight_loss(q_actual = all.sfds$avg.30day.use[[i]], q_optimal = all.sfds$avg.30day.use_optimal_elasticity326_MUC000[[i]],
                                                                                             A = all.sfds$A_elasticity326[[i]], no.sewer = all.sfds$no.sewer[[i]])}
all.sfds$DWL_optimal_elasticity326_MUC184 <- NA
for(i in 1:nrow(all.sfds)){all.sfds$DWL_optimal_elasticity326_MUC184[[i]] <- deadweight_loss(q_actual = all.sfds$avg.30day.use[[i]], q_optimal = all.sfds$avg.30day.use_optimal_elasticity326_MUC184[[i]],
                                                                                             A = all.sfds$A_elasticity326[[i]], no.sewer = all.sfds$no.sewer[[i]])}
all.sfds$DWL_optimal_elasticity326_MUC368 <- NA
for(i in 1:nrow(all.sfds)){all.sfds$DWL_optimal_elasticity326_MUC368[[i]] <- deadweight_loss(q_actual = all.sfds$avg.30day.use[[i]], q_optimal = all.sfds$avg.30day.use_optimal_elasticity326_MUC368[[i]],
                                                                                             A = all.sfds$A_elasticity326[[i]], no.sewer = all.sfds$no.sewer[[i]])}




##### sum island-wide residential DWL #####


# elasticity -0.020
sum(all.sfds$DWL_optimal_elasticity020_MUC000, na.rm = TRUE)
sum(all.sfds$DWL_optimal_elasticity020_MUC184, na.rm = TRUE)
sum(all.sfds$DWL_optimal_elasticity020_MUC368, na.rm = TRUE)

# elasticity -0.060
sum(all.sfds$DWL_optimal_elasticity060_MUC000, na.rm = TRUE)
sum(all.sfds$DWL_optimal_elasticity060_MUC184, na.rm = TRUE)
sum(all.sfds$DWL_optimal_elasticity060_MUC368, na.rm = TRUE)

# elasticity -0.326
sum(all.sfds$DWL_optimal_elasticity326_MUC000, na.rm = TRUE)
sum(all.sfds$DWL_optimal_elasticity326_MUC184, na.rm = TRUE)
sum(all.sfds$DWL_optimal_elasticity326_MUC368, na.rm = TRUE)


rm(deadweight_loss,i)

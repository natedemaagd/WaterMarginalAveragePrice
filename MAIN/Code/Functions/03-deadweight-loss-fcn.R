...
deadweight_loss <- function(q_actual, q_optimal, A, no.sewer = 'Other'){
  ...
  # set lower cutoff (minimum consumption) so value of integral doesn't approach inf
  lower_cutoff <- 500 # avg 30-day use
  
  
  # get integral from demand curve
  top_curve <- ifelse(is.na(A) | is.na(q_actual) | is.na(q_optimal) | q_actual <= lower_cutoff,
                      NA,
  ...
  if(q_actual >= lower_cutoff & q_actual < volume_water_blk1_qty & q_optimal >=0 & q_optimal < volume_water_blk1_qty & !is.na(q_actual) & !is.na(q_optimal) & q_actual >= lower_cutoff){
  ...
  } else if(q_actual >= volume_water_blk1_qty & q_actual < volume_water_blk2_qty & q_optimal >= volume_water_blk1_qty & q_optimal < volume_water_blk2_qty & !is.na(q_actual) & !is.na(q_optimal) & q_actual >= lower_cutoff) {
  ...
  } else if(q_actual >= volume_water_blk2_qty & q_optimal >= volume_water_blk2_qty & !is.na(q_actual) & !is.na(q_optimal) & q_actual >= lower_cutoff) {
  ...
  } else if(q_actual >= lower_cutoff & q_actual < volume_water_blk1_qty & q_optimal >= volume_water_blk1_qty & q_optimal < volume_water_blk2_qty & !is.na(q_actual) & !is.na(q_optimal) & q_actual >= lower_cutoff) {
  ...
  } else if(q_actual >= volume_water_blk1_qty & q_actual < volume_water_blk2_qty & q_optimal >= volume_water_blk2_qty & !is.na(q_actual) & !is.na(q_optimal) & q_actual >= lower_cutoff) {
